 ...
 *      Entity 를 분석하고, create나 insert 같은 sql 쿼리를 생성해주고, JDBC api를 사용해서 DB 접근까지 해줌. (객체와 테이블을 맵핑해줌)
 *
...
/*
    @Entity로 등록할 때 반드시 필드중에 어떤게 PK 인지 명시해줘야함
    @Id : 해당 필드가 PK라고 선언하는 애너테이션임
...
public class Ex01_1_Article_엔티티로_등록 {
    /* private Long id 의 경우 auto_increment 처럼 DB에 값을 저장하고 그 이후에 키 값을 구할건데 이걸 기본키 전략이라고 함 */
    @Id /* 기본 키(PK)와 객체의 필드를 매핑시켜주는 어노테이션
            @Id 만 사용할 경우: 키 직접 할당해야함
            기본키 값을 직접 할당하지 않고 DB가 대신 생성해주는 값을 사용하려면 @GeneratedValue 사용해야함
        */

    @GeneratedValue(strategy = GenerationType.IDENTITY) // 이게 auto_increment임
                                                        // mysql auto_increment는 IDENTITY 방식으로 만들어짐
    ...
    * @Setter 를 클래스 단위에 걸지 않고, 별도로 필요한 필드들에만 @Setter 를 달아주는걸 추천함*/
    ...
    @Setter
    @Column(nullable = false) // 해당 컬럼이 not null인 경우 @Column(nullable = false) 써줘야 함 (기본값은 true)
    private String title; // 제목 // NN, varchar(255) > length 안 쓰면 기본 255
    ...
    // 양방향 바인딩(Ex01_2의 // 연관관계 매핑. 다 끝나고 와서 하는거)
    ...
    @ToString.Exclude  /* 엄청 중요!!
                        ...
                        @ToString이 모든 필드들을 다 찍고 Set<Ex01_2_ArticleComment_엔티티로_등록> 꺼를 찍으려고 하는데 그러면 ArticleComment파일 가서도 거기에 있는 @ToString이 모든 원소들을 다 찍으려고 하면서 Ex01_1_Article_엔티티로_등록 이라는걸 보는 순간 다시 Article 의 @ToString이 동작하면서 또 모든 원소들을 찍을거임. 이런식으로 서로가 서로를 호출하면서 순환 참조를 하게 되면서 메모리가 터져서 시스템이 다운될 수 있다.
    ...
    /*
        jpa auditing: JPA 에서 자동으로 셋팅하게 해줄 때 사용하는 설정
    ...
    */
    ...
    /* 이렇게 어노테이션만 붙여주기만 해도 auditing이 작동한다.
        @CreatedDate: 최초의 insert 할 때 자동으로 한 번 넣어준다.
        @CreateBy: 최초의 insert 할 때 자동으로 한 번 넣어준다.
        @LastModifiedDate: 작성 당시의 시간을 매번 실시간으로 넣어준다
        @LastModifiedBy : 작성 당시의 이름을 매번 실시간으로 넣어준다

        위 4개 중에 생성일시나 수정일시는 알아낼 수 있는데, 최초의 생성자는 테스트시 로그인을 하고 온게 아니기 때문에 따로 알아낼 수가 없다.
        그래서 config > JpaConfig 의  public AuditorAware<String> auditorAware() {}를 사용한다.
        리턴타입이 String이기 때문에 createBy나 modifiedBy 에 들어갈 수 있다.
    *  */
    ...
    /* factory method pattern (정적 팩토리 메서드)
        정적 팩토리 매서드는 객체 생성의 역할을 하는 클래스 메서드. (static 으로 무조건 해야함)
        of 메서드를 이용해서 직접적으로 생성자를 사용해서 객체를 생성함

        장점: 1) static 이기 때문에 new 를 이용하지 않아도 생성자를 만들 수 있다.
             2) return 을 가지고 있기 때문에 상속을 사용할 때 값을 확인할 수 있다.(한마디로 하위 자료형 객체를 반환할 수 있다)
             3) 객체 생성을 캡슐화 가능
...
//        return Objects.equals(id, that.id);
    ...
    // 여기 오버라이드 안 하면 Object에 있는거 쓸건데 id 어찌보내줄거임?
    // 그래서 그냥 여기에 오버라이드해서 id 보내기
    @Override
    ...
    /*
        ...
        hashcode - 객체를 식별하는 Integer 값
                    ...
                    hashcode 사용하는 이유는 객체를 비교할 때 드는 비용이 낮다
                    자바에서 2개의 객체가 같은지 비교할 때 equals() 를 쓰는데
                    여러개의 객체를 배교할 때 equals를 사용하면 Integer 값들을 비교할 때 많은 시간과 비용이 발생함
    */
}
