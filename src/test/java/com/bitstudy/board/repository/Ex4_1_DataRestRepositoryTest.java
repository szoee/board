...
/* 컨트롤러에 대한 테스트(슬라이스 테스트)
    ...
    - 대표적인 슬라이스 테스트 어노테이션
        1) @WebMvcTest - Controller 를 테스트 할 때 주로 사용
                        @WebMvcTest 선언하면 Web과 관련된 Bean만 주입이 되고. MockMvc 를 알아볼 수 있게됨
                        *MockMvc는 웹 어플리케이션을 서버에 배포하지 않고 테스트용 MVC 환경을 만들어서 요청 및 전송
                        *쉽게 말하면 내가 컨트롤러 테스트를 하고 싶을 때 실제 서버에 구현된 어플리케이션을 올리지 않고 테스트용 시뮬레이션을 돌려서 MVC가 되도록 도와주는 클래스
...
public class Ex4_1_DataRestRepositoryTest {
    /* MockMvc 테스트 방법
        ...
        4) Expect가 모두 통과하면 통과
            하나라도 실패하면 실패

        Hal에서 http://localhost:8080/api/article 를 돌려보면 동작하는데 왜 여기서는 안되냐면
        @WebMvcTest는 슬라이스 테스트이기 때문에 컨트롤러 이외의 빈은 아예 로그하지 않기 때문임
    ...
    void test1() throws Exception {
        ...
        // when & than
        mockMvc.perform(get("/api/articles")) // ctrl + space > alt + enter
                .andExpect(status().isOk()) // 현재 상태가 200인가 (존재하냐? 라고 물어보는거)
                .andExpect(content().contentType(MediaType.valueOf("application/hal+json")));

        /* get() 작성하는 법
            1) perform()안에 get이라고 친다.
            2) 추천에 getClass만 뜰건데 그 상태에서 Ctrl + space 누른다
            3) 추천 리스트가 뜨는데 그 중에 MockMvcRequestBuilders 있을거임. 그거 alt + 엔터 선택
                윈도우: alt + enter
                맥: 옵션 + 엔터
                이렇게하면 get() 부분 가져와진다 (static import 방식)
